...
public class areaDao58 extends Dao {

	public areaDao58(Database db) {
		super(db);
	...
	public String getAreaId(String lon, String lan) {
		...
		try {// 获取区域四个角对象
			...
			//	System.out.println("搜索区域号"+areaId);
				...
				polygonLan.add(Double.parseDouble(rtlan));//&&lon<=rtlon&&lblan<=lan&&lan<=rtlan
				boolean flag=false;
				if(Double.parseDouble(lblon)<=Double.parseDouble(lon)&&Double.parseDouble(lon)<=Double.parseDouble(rtlon)
						&&Double.parseDouble(lblan)<=Double.parseDouble(lan)&&Double.parseDouble(lan)<=Double.parseDouble(rtlan)){
					flag=true;
				}
