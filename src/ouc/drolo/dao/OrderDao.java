...
public class OrderDao extends Dao {
	...
	public int addonestaffInfo(String name, String pswd, String phone,
		...
		String sql="select id from _staff where staffid='"+staffid+"'";
		...
		f = db.executeUpdate(sqlString, itemList);
		}
			...
			public int handle(ResultSet rs) throws SQLException {
				...
				try {
					while (rs.next()) {
						...
						String sta;
						if(rs.getString("orderId").equals("201409260002")){
							logger.debug("xxx  " + rs.getString("status")); 
						}
						...
						if (rs.next()) {
							try {
								...
								str.key("name").value(rs.getString("name"))
									...
									.key("ereaNun").value(rs.getString("ereaNun"))
						.endObject().endArray();
	...
	public String findOrderByOrderId(String orderId) {
		...
		String sql = "SELECT o.orderId AS orderId, o.large AS large,o.amount AS amount,o.address AS address,o.takePhone AS takePhone,o.note AS note,o.userId AS userId,"
				+ "o.generateTime AS generateTime,o.status AS status,o.userName AS userName,o.staffId as staffId ,"
		...
		try {
			...
			if (rs.next()) {
				...
				jsonObject.put("staffId", rs.getString("staffId") + "");
